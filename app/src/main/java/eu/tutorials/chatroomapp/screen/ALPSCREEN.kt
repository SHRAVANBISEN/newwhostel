...
fun ALPVIEW(
    ...
    LaunchedEffect(Unit) {
        viewModel.loadWishesByHostel("ALP")
    }
    ...
    Scaffold(
        ...
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = {
                        ...
                        navController.navigate(Screen.PrincipleScreen.route) {
                            popUpTo(Screen.ALPHOSTEL.route) { inclusive = true }
                ...
                title = { Text(text = "ALP Problems", color = colorResource(id = R.color.white)) },
                actions = {
                    IconButton(onClick = {
...
@Composable
fun ALPWISHITEM(wish: Wish, onClick: () -> Unit) {
    Card(
