...
fun AKKVIEW(
    ...
    LaunchedEffect(Unit) {
        viewModel.loadWishesByHostel("AKM")
    }
    ...
    Scaffold(
        ...
        topBar = {
            CenterAlignedTopAppBar(
                ...
                navigationIcon = {
                    IconButton(onClick = {
                        ...
                        navController.navigate(Screen.PrincipleScreen.route) {
                            popUpTo(Screen.AKKAM.route) { inclusive = true }
                ...
                title = { Text(text = "Hostel 1 Problems", color = colorResource(id = R.color.white)) },
                actions = {
                    IconButton(onClick = {
...
@Composable
fun AKKAWishItem(wish: Wish, onClick: () -> Unit) {
    Card(
